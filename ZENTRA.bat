...
rem
rem  นี่คือไฟล์เดียวที่ผู้ใช้ต้องกด ครั้งแรกจะติดตั้งให้อัตโนมัติแล้วเปิดต่อ
rem  ครั้งต่อ ๆ ไปจะเปิดโปรแกรมทันที
rem
rem  หมายเหตุสำหรับผู้พัฒนา:
rem  - .gitattributes บังคับให้ไฟล์นี้เป็น CRLF เสมอ (ทั้ง clone และ ZIP)
...
if exist ".venv\Scripts\python.exe" goto ENV_SETUP

rem ── ยังไม่ได้ติดตั้ง -> ติดตั้งให้เลย ไม่ต้องให้ผู้ใช้ไปกดไฟล์อื่น ──────
echo.
echo ============================================================
echo  ยินดีต้อนรับสู่ ZENTRA - เปิดใช้งานครั้งแรก
echo ============================================================
echo.
echo ยังไม่ได้ติดตั้งระบบในเครื่องนี้ จะติดตั้งให้อัตโนมัติเลยครับ
echo ทำครั้งเดียว ครั้งต่อไปกดไฟล์นี้แล้วเปิดได้ทันที
echo.
if not exist "INSTALL.bat" goto NO_INSTALLER
call "%~dp0INSTALL.bat" /auto
if not exist ".venv\Scripts\python.exe" goto INSTALL_FAILED
echo.
echo ------------------------------------------------------------
echo  ติดตั้งเสร็จแล้ว - กำลังเปิดโปรแกรม
echo ------------------------------------------------------------
echo.
...
echo.
echo ============================================================
echo  [ผิดพลาด] ติดตั้งไม่สำเร็จ
echo.
echo  ดูข้อความผิดพลาดด้านบน - ส่วนใหญ่แก้ได้ด้วยการเช็คอินเทอร์เน็ต
echo  แล้วดับเบิลคลิก ZENTRA.bat ใหม่อีกครั้ง
echo  (ของที่ติดตั้งไปแล้วจะไม่ทำซ้ำ เริ่มต่อจากจุดที่ค้างไว้)
echo ============================================================
echo.
pause
exit /b 1
...
echo.
echo ============================================================
echo  [ผิดพลาด] ไม่พบไฟล์ INSTALL.bat
echo.
echo  ไฟล์โปรเจกต์แตกออกมาไม่ครบ
echo  ถ้าโหลดมาเป็น ZIP ให้คลิกขวาที่ไฟล์ ZIP -^> Extract All
echo  แล้วเปิด ZENTRA.bat จากโฟลเดอร์ที่แตกออกมา
echo ============================================================
echo.
pause
exit /b 1
